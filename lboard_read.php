...
?>
                <a href="lboard_main.php"><button class="btn4">Back to board</button></a><br>
            <form action="lmessage.php">
    ...
    while($row = mysql_fetch_array($result))
    {
        ...
        if($row[imageUp] != null)
            echo "<tr><td align=center><img src='luploads/$row[imageUp]' height='500px' width='500px'></td></tr>";
        echo "</table>";
    }
...
?>
        ...
        <form name="write" method="post" action="lboard_write.php">
     <input class="btn4" type="submit" value="Write Article">
    </form>
    <form method="post" action="homeafterlogin.php">
     <input class="btn4" type="submit" value="Homepage">
    </form>
    <form name="logout" method="post" action="login.php?mode=logout">
     <input class="btn4" type="submit" value="Log Out">
    </form>
    <? 
        if($article_owner == $_SESSION['user']){
        ?>
        <a href='ltext_delete.php?num=<?php echo $num ?>'><button class='btn4' onclick="checkPasword()">Delete</button></a>
        <?
        }
    ?>
            </center>
